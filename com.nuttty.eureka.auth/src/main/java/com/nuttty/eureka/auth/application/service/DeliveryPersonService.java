...
public class DeliveryPersonService {
    ...
    public DeliveryPersonInfoDto createDeliveryPerson(UserRoleEnum role, Long userId, DeliveryPersonCreateDto createDto) {
        // 회원 가입 유무 검사
        if (!userRepository.existsById(userId)) {
            throw new EntityNotFoundException("등록되지 않은 유저입니다.");
        }
    ...
    public DeliveryPersonInfoDto getDeliveryPersonInfo(UserRoleEnum role, Long userId, Long deliveryPersonId) {
        // 배송 담당자 등록 여부 검사
    ...
    public DeliveryPersonInfoDto updateDeliveryPersonType(UserRoleEnum role, Long userId, Long deliveryPersonId, DeliveryPersonTypeUpdateRequestDto updateDto) {
        // 배송 담당자 등록 여부 검사
        DeliveryPerson deliveryPerson = deliveryPersonRepository.findById(deliveryPersonId)
                .orElseThrow(() -> new EntityNotFoundException("등록되지 않은 배송 담당자입니다."));
    ...
    public String deleteDeliveryPerson(UserRoleEnum role, Long userId, String email, Long deliveryPersonId) {
        // 배송 담당자 등록 여부 검사
        DeliveryPerson deliveryPerson = deliveryPersonRepository.findById(deliveryPersonId)
                .orElseThrow(() -> new EntityNotFoundException("등록되지 않은 배송 담당자입니다."));
        ...
        if (deliveryPerson.getIsDelete()){
            throw new AlreadyIsDeletedException("이미 배송담당자를 탈퇴했습니다.");
        }
    ...
    // 배송 담당자 전체 조회
    @Transactional(readOnly = true)
    @Cacheable(cacheNames = "deliveryPersonSearchInfoCache", key = "#role + ':' + #searchRequestDto.user_id + ':' + #searchRequestDto.delivery_person_id + ':' + #searchRequestDto.hub_id + ':' + #searchRequestDto.slack_id + ':' + #searchRequestDto.type")
    public Page<DeliveryPersonSearchResponseDto> searchDeliveryPerson(UserRoleEnum role, Pageable pageable, DeliveryPersonSearchRequestDto searchRequestDto) {
